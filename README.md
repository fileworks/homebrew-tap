...
repository, workflow run, and immutable `uv.lock` URL/SHA-256 provenance. The
...
result is present on `main`. Equal versions are idempotent no-ops; lower
versions are recorded as stale and cannot roll a formula back.
...
Complete formulas are generated by the release pipeline — see
[CONTRIBUTING.md](CONTRIBUTING.md) before editing one by hand. Structural
changes are ordinary pull requests validated with `brew style`,
`brew audit --strict`, and package-index-disabled install and inventory tests.
